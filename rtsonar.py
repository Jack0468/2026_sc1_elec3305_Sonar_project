...
def signal_process( Qin, Qdata, pulse_a, Nseg, Nplot, fs, maxdist, temperature, functions, stop_flag  ):
    ...
    # FIX: Use np.minimum to avoid AxisError when using 'from numpy import *'
    maxsamp = np.minimum(int(dist2time( maxdist, temperature) * fs), Nseg) # maximum samples corresponding to maximum distance
...
def image_update( Qdata, fig, Nrep, Nplot, stop_flag, handle):
    ...
    while(  not stop_flag.is_set() ):
        ...
        # FIX: Pass the notebook handle explicitly
...
def rtsonar( f0, f1, fs, Npulse, Nseg, Nrep, Nplot, maxdist, temperature, functions ):
    ...
    # FIX: Capture the notebook handle
    ...
    t_rec = threading.Thread(target = record_audio,   args = (Qin, p, fs, stop_flag, 1  ))
    ...
    # FIX: Pass the captured handle into the image update thread
